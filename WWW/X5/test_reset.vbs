...
blnResult = Ping(strSubNet)  '调用自定义Ping函数来试试上面的IP的机器是否开机,返回一个布尔值,前提是对方机器没有防火墙等限制.
    If blnResult = True Then  
        ...
        WshShell.SendKeys("{ENTER}")
        WScript.Sleep 1000
        WshShell.SendKeys "uci set test.test.start_num=0"
        WshShell.SendKeys("{ENTER}")
        WScript.Sleep 1000
        WshShell.SendKeys "uci set test.test.hub_faile=0"
        WshShell.SendKeys("{ENTER}")
        WScript.Sleep 1000
        WshShell.SendKeys "uci set test.test.hub_ok=0"
        WshShell.SendKeys("{ENTER}")
        WScript.Sleep 1000
        WshShell.SendKeys "uci commit test"
        WshShell.SendKeys("{ENTER}")
         WScript.Sleep 1000
